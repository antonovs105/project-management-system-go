...
func TestRESTDTOsUseStableSnakeCaseFields(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "remote project",
			value: apfederation.RemoteProject{
				ID:             "remote-project-1",
				ProjectAPID:    "https://remote.test/projects/board",
				ProjectName:    "Remote Board",
				Role:           "developer",
			...
			wantKeys: []string{
				"id", "project_ap_id", "project_name", "role", "target_inbox_url", "inviter_actor_id",
				"inviter_ap_id", "inviter_handle", "inviter_name", "created_at", "updated_at",
			},
			forbidKeys: []string{"projectAPID", "projectName", "targetInboxURL", "inviterActorID"},
		},
		{
			name: "remote ticket write result",
			value: apfederation.RemoteTicketWriteResult{
				Ticket: &apfederation.RemoteTicket{
					ID:          "encoded-ticket",
					APID:        "https://remote.test/tickets/1",
					Title:       "Remote ticket",
					...
					ReporterID:  "encoded-reporter",
					CreatedAt:   now,
					UpdatedAt:   now,
				},
				Delivery: &apfederation.FollowDelivery{
					ID:             "delivery-1",
					ActivityAPID:   "https://local.test/activities/1",
					TargetInboxURL: "https://remote.test/projects/board/inbox",
					State:          "pending",
					CreatedAt:      now,
					UpdatedAt:      now,
			...
			wantKeys:   []string{"ticket", "delivery"},
			forbidKeys: []string{"targetInboxURL", "activityAPID"},
		},
		{
			name: "ticket",
			value: ticket.Ticket{
