WITH invite_scoped_deliveries AS (
    SELECT
        delivery.id,
        COALESCE(
...
SET project_actor_id = NULL
FROM invite_scoped_deliveries scoped
WHERE scoped.id = delivery.id
    AND scoped.existing_scope IS NULL
    AND scoped.invite_scope IS NOT NULL
    AND delivery.project_actor_id = scoped.invite_scope;
